...
from app.models import Badges, Visits
...
def find_visit_by_badge(
    ...
    visit = (
        ...
        .join(Visits.visitor)
        .filter(
            Visits.badge_id == badge.badge_id, Visits.status == VisitStatus.CHECKED_IN
        )
        .first()
    )
    ...
    return {
        ...
        "total_time": int(
            (datetime.now(timezone.utc) - visit.check_in_time).total_seconds() // 60
        ),
