...
def get_available_badges(
    ...
    return (
        db.query(Badges)
        .filter(Badges.badge_status == BadgeStatus.AVAILABLE)
        .limit(30) # to avoid fetching large number of badges, we can set a limit of 50 badges for now, we can change it later if needed
        .all()
    )